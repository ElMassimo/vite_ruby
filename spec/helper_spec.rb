...
protected
  ...
  def assert_similar(*args)
    assert_equal(*args.map { |str| str.tr("\n", "") })
  end
...
# Helper to run an ActionView::TestCase method inside RSpec
def run_helper_test(klass, test_name)
  t = klass.new(test_name.to_s)
  t.setup
  t.public_send(test_name)
