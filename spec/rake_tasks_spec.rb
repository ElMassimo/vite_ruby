...
RSpec.describe "ViteRuby rake tasks" do
  it "rake_tasks" do
    expect(ViteRuby.install_tasks).to be_truthy
    output = Dir.chdir(path_to_test_app) { `#{RAKE_BIN} -T` }

    expect(output).to include("vite:build")
    expect(output).to include("vite:build_ssr")
    expect(output).to include("vite:clobber")
    expect(output).to include("vite:install_dependencies")
    expect(output).to include("vite:verify_install")
  end

  it "rake_task_vite_check_binstubs" do
    output = Dir.chdir(path_to_test_app) { `#{RAKE_BIN} vite:verify_install 2>&1` }
    expect(output).not_to include("vite binstub not found.")
  end

  it "rake_vite_install_dependencies_in_non_production_environments" do
    expect(test_app_dev_dependencies).to include("right-pad")
    ...
    expect(installed_node_module_names).to include("right-pad")
  end
